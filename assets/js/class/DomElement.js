...
class DomElement {
    constructeur(selectedElement, addedNodeElement, ...attributes) {
        ...
        this.addedNodeElement = addedNodeElement;
        this.attributes = attributes;
    }

    injectedElement() {
        const domElement = document.querySelector(this.selectedElement).appendChild(document.createElement(this.addedNodeElement));
    ...
    */
    static domHandlerSection() {
        ...
        const h2 = new DomElement("section:nth-child(2)", "h2");
        h2.injectedElement();
        const span = new DomElement(h2, "span", [{ "class": "secondary-color" }]);
        span.injectedElement();
        const spanText = new DomElement(span, `${data[0].at(1)[0]}`);
        spanText.injectedText();
        const h2Text = new DomElement(h2, `${data[0].at(1)[1]}`);
        h2Text.injectedText();
        ...
        articles.forEach((article, i) => {
            ...
            article.injectedElement();

            /*Création de l'élément h3 dans l'article*/

            const h3 = new DomElement(`${article}`, "h3");
            h3.injectedElement();
            const icon1 = new DomElement(`${article} h3`, "i", [{ "class": "fa-solid fa-droplet" }, { "aria-hidden": "true" }]);
            icon1.injectedElement();
            //attention aria-hidden en camelCase, vérif sur console.log
            const h3Text = new DomElement(`${article} h3`, `${data[i + 1].at(1).title}`);
            h3Text.injectedText();
            const icon2 = new DomElement(`${article} h3`, "i", [{ "class": "fa-solid fa-droplet" }, { "aria-hHidden": "true" }]);
            icon2.injectedElement();

            /*Création de l'élément img dans l'article*/

            const image = new DomElement(`${article}`, "img", [{ "src": `${data[i + 1].at(1).img.url}` }, { "alt": `${data[i + 1].at(1).img.alt}` }]);
            image.injectedElement();

            /*Création des éléments p dans l'article*/

            const para = [];

            para.forEach((p, i) => {
                p = new DomElement(`${article}`, "p");
                p.injectedElement();
                const pText = new DomElement(`p:nth-child(${i + 1})`, `${data[i + 1].at(1).para[0].at(i)}`);
                pText.injectedText();
            });

            /*Création de l'élément a dans l'article*/

            const a = new DomElement(`${article}`, "a", [{ "href": "#" }, { "aria-hidden": "true" }]);
            const icon3 = new DomElement(`${article} a`, "i", [{ "class": "fa-solid fa-circle-plus" }, { "aria-hidden": "true" }]);
            a.injectedElement();
            const aText = new DomElement(`${article} a`, `${data[i + 1].at(1).a}`);
            aText.injectedText();
            icon3.injectedElement();
